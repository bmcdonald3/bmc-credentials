...
// LoadAllBmcs loads all Bmc resources from Ent storage
func LoadAllBmcs(ctx context.Context) ([]*v1.Bmc, error) {
	...
	entResources, err := entClient.Resource.Query().
		Where(entresource.KindEQ("Bmc")).
		WithLabels().
		WithAnnotations().
		All(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to load Bmc resources: %w", err)
	}

	// Convert to Fabrica resources
	var resources []*v1.Bmc
	for _, entResource := range entResources {
		...
		resources = append(resources, fabricaResource.(*v1.Bmc))
	}

	return resources, nil
}

// LoadBmc loads a single Bmc resource by UID from Ent storage
func LoadBmc(ctx context.Context, uid string) (*v1.Bmc, error) {
	...
	return fabricaResource.(*v1.Bmc), nil
}

// SaveBmc saves a Bmc resource to Ent storage
func SaveBmc(ctx context.Context, resource *v1.Bmc) error {
	if entClient == nil {
	...
	if err != nil {
		return fmt.Errorf("failed to convert Bmc to ent: %w", err)
	}
...
// DeleteBmc deletes a Bmc resource from Ent storage
...
func (b *entStorageBackend) Save(ctx context.Context, resourceType, uid string, data json.RawMessage) error {
	...
	case "Bmc":
		...
		if err := json.Unmarshal(data, &res); err != nil {
			return fmt.Errorf("failed to unmarshal Bmc: %w", err)
		}
		res.Metadata.UID = uid
		return SaveBmc(ctx, &res)
	default:
