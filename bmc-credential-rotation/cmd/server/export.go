...
Examples:
  ...
  bmc_credential_rotation export --format yaml --output ./backup

  # Export specific resource types
  bmc_credential_rotation export --kinds Bmc --output ./bmc-backup
`,
...
func exportResourceKind(ctx context.Context, kind, output, format string, perType bool) (int, error) {
	...
	switch kind {
	case "Bmc":
		items, e := storage.Querybmcs(ctx).All(ctx)
		if e != nil {
			return 0, fmt.Errorf("failed to query bmcs: %w", e)
		}
