...
// This file provides the generated boilerplate for Bmc reconciler.
//
...
//
// To customize reconciliation logic, edit bmc_reconciler.go
package reconcilers
...
// BmcReconciler reconciles Bmc resources.
//
// This reconciler:
//   - Observes Bmc resources and updates their Status
...
//
// The implementation of reconcileBmc() is in bmc_reconciler.go
type BmcReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in bmc_reconciler.go
}

// NewDefaultBmcReconciler creates a default Bmc reconciler.
//
...
//
// Returns:
//   - *BmcReconciler: Initialized reconciler
...
func (r *BmcReconciler) GetResourceKind() string {
	return "Bmc"
}

// Reconcile brings Bmc to desired state.
//
// This method is called:
//   - When a Bmc resource is created/updated/deleted
...
//   - resource: The Bmc resource to reconcile
//
// Returns:
...
func (r *BmcReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.Bmc // This is the typed struct
	...
	r.Logger.Debugf("Reconciling Bmc %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileBmc(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for Bmc %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for Bmc %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.bmcs.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for Bmc %s: %v", res.GetUID(), err)
